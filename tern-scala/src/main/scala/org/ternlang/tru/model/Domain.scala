...
case class Domain(version: Version, scope: Scope) {
  ...
  def addSourceUnit(path: String, qualifier: String): SourceUnit = sources.addSourceUnit(path, qualifier)
  ...
  private class NamespaceSet(domain: Domain) {
    ...
    def addNamespace(name: String): Namespace = {
      val namespace = namespaces.computeIfAbsent(name, _ => new Namespace(domain, name))
