...
class ServiceTrait(domain: Domain, entity: Entity, mode: Mode) extends Template(domain, entity, mode) {
  ...
  override protected def generateEntity(): Unit = {
    ...
    builder.append(s"${category} ${getName} {")
    generateBody()
    builder.append("}\n")
  }
  ...
  override protected def generateBody(): Unit = {
    ...
    properties.forEach(property => {
      ...
      builder.append(s"   def if${method}(${identifier}: (${constraint}) => Unit): Unit\n")
    })
    generateMatchMethod()
