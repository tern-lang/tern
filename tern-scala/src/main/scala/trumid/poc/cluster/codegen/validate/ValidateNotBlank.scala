...
class ValidateNotBlank(domain: Domain,
                       entity: Entity,
                       property: Property,
                       mode: Mode,
  ...
  def generateValidation(builder: SourceBuilder): Unit = {
    ...
    if(property.isOptional()) {
      builder.append(s"      if(${parameter}.${identifier}().isDefined && ${parameter}.${identifier}().get.length() == 0) {\n")
      builder.append("         return ResultCode.fail(\"Invalid value for '" + identifier + "'\") // " + origin + "\n")
      builder.append(s"      }\n")
    } else {
      builder.append(s"      if(${parameter}.${identifier}().length() == 0) {\n")
      builder.append("         return ResultCode.fail(\"Invalid value for '" + identifier + "'\") // " + origin + "\n")
      builder.append(s"      }\n")
    }
  }
